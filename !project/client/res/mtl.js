...
class _mtl {
  constructor(program, ka, kd, ks, ph, trans, texNames, isShade) {
    ...
    globalGL.uniform1i(
      globalGL.getUniformLocation(this.program, "isTex0"),
      this.texInd[0]
    );
    globalGL.uniform1i(
      globalGL.getUniformLocation(this.program, "isTex1"),
      this.texInd[1]
    );
    globalGL.uniform1i(
      globalGL.getUniformLocation(this.program, "isTex2"),
      this.texInd[2]
    );
    globalGL.uniform1i(
      globalGL.getUniformLocation(this.program, "isTex3"),
      this.texInd[3]
    );
    ...
    if (ka != undefined)
      globalGL.uniform3fv(kaLoc, new Float32Array(ka.toArray()));
    else globalGL.uniform3fv(kaLoc, new Float32Array(vec3().toArray()));
    if (kd != undefined)
      globalGL.uniform3fv(kdLoc, new Float32Array(kd.toArray()));
    else globalGL.uniform3fv(kdLoc, new Float32Array(vec3().toArray()));
    if (ks != undefined)
      globalGL.uniform3fv(ksLoc, new Float32Array(ks.toArray()));
